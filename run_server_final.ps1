...
if ($LASTEXITCODE -eq 0) {
    ...
    $pid8893 = netstat -ano | findstr :8893 | ForEach-Object { $_.Split(' ', [System.StringSplitOptions]::RemoveEmptyEntries)[-1] } | Select-Object -First 1
    if ($pid8893) {
        Write-Host "Killing process on 8893: $pid8893"
        taskkill /F /PID $pid8893
    }
    $JAVA_OPTS = "-Dorg.apache.tomcat.util.descriptor.tld.TldParser.skipValidation=true -Dorg.apache.jasper.compiler.TldLocationsCache.noTldNextTime=true"
    java $JAVA_OPTS -cp "$CP" test.SimpleServer
} else {
